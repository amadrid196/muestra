<template>
    <main class="col-sm-9 offset-sm-3 col-md-10 offset-md-2 pt-3">
        <div class="col-12 col-md-12">
            <h6><router-link to="/menu">menu</router-link> \ <router-link to="/menu/tipoEvaluacion">evaluaciones</router-link> \ {{title}}</h6>
            <div class="card">
                <div class="card-body">
                <div v-if="isloading" class="row vh-50 align-items-center  justify-content-center">
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            <label for="inputEmail4">Nombre de evaluacion </label>
                    ...
                    <div class="form-row">
                        <div class="form-group pr-2">
                            <router-link to="/menu/tipoEvaluacion" class="btn btn-danger text-white">Volver</router-link>
                        </div>

                        <div class="form-group" v-if="!isloading && tipoAccion== 1">
                            <button type="submit" class="btn btn-primary" v-on:click="posttipoEvaluacion">Guardar</button>
                        </div>
                        <div class="form-group" v-if="!isloading && tipoAccion== 2">
                            <button type="submit" class="btn btn-primary" v-on:click="puttipoEvaluacion">Guardar</button>
                        </div>
                    </div>
                </div>
            </div>
...
methods:{
    ...
    getData: function(){
        if(this.$route.params.data){
            console.log('edit',this.$route.params.data)
            this.idtipoEvaluacion = this.$route.params.data.id_tipo_evaluacion;
    ...
    posttipoEvaluacion: function(){
    if (this.checkForm()) {
        return;
      }
    this.isloading = true
    var url = "/tipoEvaluacionPost";
      axios
        .post(url, {
          name: this.name,
        ...
        if(response.data.success == true){
            this.$router.push('/menu/tipoEvaluacion')
        }
        });
    },

    puttipoEvaluacion: function(){
    if (this.checkForm()) {
        return;
      }
    this.isloading = true
    var url = "/puttipoEvaluacion";
      axios
        .put(url, {
          name: this.name,
