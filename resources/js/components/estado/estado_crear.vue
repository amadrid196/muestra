<template>
    <main class="col-sm-9 offset-sm-3 col-md-10 offset-md-2 pt-3">
        <div class="col-12 col-md-12">
            <h6><router-link to="/menu">menu</router-link> \ <router-link to="/menu/estado">estado</router-link> \ {{title}}</h6>
            <div class="card">
                <div class="card-body">
                <div v-if="isloading" class="row vh-50 align-items-center  justify-content-center">
                    ...
                    <div class="form-row">
                        <div class="form-group pr-2">
                            <router-link to="/menu/estado" class="btn btn-danger text-white">Volver</router-link>
                        </div>
                        <div class="form-group" v-if="!isloading && tipoAccion== 1">
                            <button type="submit" class="btn btn-primary" v-on:click="postEstado">Guardar</button>
                        </div>
                        <div class="form-group" v-if="!isloading && tipoAccion== 2">
                            <button type="submit" class="btn btn-primary" v-on:click="putEstado">Guardar</button>
                        </div>
                    </div>
                </div>
            </div>
...
methods:{
    ...
    getdataEstado: function(){
        if(this.$route.params.data){
            this.idEstado = this.$route.params.data.id_estado;
