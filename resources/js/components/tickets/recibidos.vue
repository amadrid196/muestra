...
<router-link :to="{name: 'recibidos', params: { data: item}}"  class="text-white nav-item nav-link col-sm-12 col-md-3 d-inline p-3" >
...
methods: {
    getTicketRecibido: function() {
        axios.get('/getTicketRecibido').then((response)=>{
            console.log('data',response)
            this.documentos = response.data.recibido;
        }).catch((err) => {
          console.log(err);
        });
    },
